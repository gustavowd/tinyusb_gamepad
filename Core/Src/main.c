...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define USBD_STACK_SIZE    (8*configMINIMAL_STACK_SIZE/2) * (CFG_TUSB_DEBUG ? 2 : 1)
#define HID_STACK_SZIE      4*configMINIMAL_STACK_SIZE
...
bool debounceGpioGetAll(gamepad_t *gamepad){
	...
	if (gamepad->debounced_gpio == (raw_gpio & 0xF7FB)) {
